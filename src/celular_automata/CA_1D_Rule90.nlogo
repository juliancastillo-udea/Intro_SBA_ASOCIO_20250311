globals [row]  ;; Variable encargada del proceso de filas.
patches-own [left-pcolor center-pcolor right-pcolor] ; Cada parche tiene tres opciones, iz, cen, der
to setup ; Inicialización del modelo
  clear-all ; Limpia todo el entorno visual y de variables
  set row max-pycor ; Define el valor de fila como el máximo valor en Y
  ask patch 0 max-pycor [ set pcolor yellow ]  ; Define en color amarillo el centro en X, la configuración del mundo establece el centro
  reset-ticks ;Reinicia el control del tiempo del modelo
end
to go ; ejecuta el modelo
  if row = min-pycor [stop]  ; Si llega al final del mundo se detiene
  ask patches with [pycor = row] ; Pregunta a todos los parches cuya fila sea igual a row
    [ do-rule ] ; ejecuta la función do-rule
  set row (row - 1) ;Pasamos a la siguiente fila
  tick ; avanzamos en el tiempo del modelo
end
to do-rule  ; Regla para los parches y aplica la regla 90
  set left-pcolor [pcolor] of patch-at -1 0 ; asigna el color de la celda izquierda a la variable
  set center-pcolor pcolor ; asigna el color de la celda centro a la variable
  set right-pcolor [pcolor] of patch-at 1 0 ; asigna el color de la celda derecha a la variable
  ; Se evalua la regla 90 en donde evalua si el color es basado en la receta
  ; 000->0, 001->1, 010->0, 011->1, 100->1, 101->0, 110->1, 111->0 (Cero negro, 1 amarillo --> yellow)
  ifelse ((left-pcolor = yellow and center-pcolor = yellow and right-pcolor = yellow) or
    ...
    [ ask patch-at 0 -1 [ set pcolor black ] ] ;Condiciones son al menos una verdadera
    [ ask patch-at 0 -1 [ set pcolor yellow ] ] ;
end
to setup-continue ; Configuracion para ejecutar la siguiente visualizacion
  ask patches with [pycor = max-pycor] ;; Copia el fondo al inicio
    [ set pcolor ([pcolor] of patch pxcor min-pycor) ]
  ask patches with [pycor != max-pycor]  ;; Limpia el resto de la vista y podemos continuar
    [ set pcolor black ]
  set row max-pycor  ;; Actualiza el valor mayor
end
