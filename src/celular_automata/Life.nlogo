globals [erasing?] ; Variable global booleana que permite dibujar las celdas
patches-own [
  living?         ; Variable de parche que indica si la celda esta viva o muerta (True, False)
  live-neighbors  ; Variable de parche que cuenta cuantos vecinos Moore están vivos
]
to setup-blank ; Configuracion inicial en blanco, permite dibujar
  clear-all ; Limpia todo
  ask patches [ cell-death ] ; Pregunta a todos los parches y aplica la funcion cell-death
  reset-ticks ; Reinicia el reloj
end
to setup-random ; Configuracion inicial aleatoria en donde se define
  clear-all ; Limpia todo
  ask patches ; Preguntamos a todos los parches (Aleatoriamente)
    [ ifelse random-float 100.0 < initial-density ; Evalua una variable aleatoria entre 0 y 100 y compara con la variable initial-density
      [ cell-birth ] ; La instruccion del condicional es True, random-float es menor a initial-density
      [ cell-death ] ] ; La instruccion del condicional es False, random-float es mayor a initial-density
  reset-ticks ; Reiniciamos el tiempo del modelo
end
to cell-birth ; Método para que una celda viva
  set living? true ; Variable booleana pasa a la vida
  set pcolor fgcolor ; Asigna el color de vida
end
to cell-death ; Método para que una celda viva
  set living? false  ; Variable booleana pasa a la muerte
  set pcolor bgcolor ; Asigna el color de muerte
end
to go ; ejecutar el modelo
  ask patches ; preguntamos a todos los parches aleatoriamente
    [ set live-neighbors count neighbors with [living?] ]
  ;Preguntamos a todos los parches y a la variable live-neighbors con set asignamos cuantos vecinos estan vivos
  ; contando con count neigghbors with [living?], estado vivo
  ;Esto mantiene todos los parches sincronizados entre sí, por lo que los nacimientos y muertes en cada generación ocurren al mismo tiempo.
  ask patches ; Ahora ejecutamos la vida o la muerte de las celdas de acuerdo a las reglas.
    [ ifelse live-neighbors = 3 ; Si es exactamente igual a 3, la celda vive
      [ cell-birth ] ; Funcion vivir
      [ if live-neighbors != 2 ; si es diferente de dos muerte, pero con la clausula anterior como limite, en este caso muerte
        [ cell-death ] ] ] ; Funcion muerte
  tick ; Pasa el tiempo del modelo
end
to draw-cells ; Dibujar las celdas usando la opcion mouse down, o revisar el mouse por el entorno
  ifelse mouse-down? [; Si la variable erasing es cero, en este caso vuelve a la vida
    if erasing? = 0 [ set erasing? [living?] of patch mouse-xcor mouse-ycor ]
    ask patch mouse-xcor mouse-ycor [; Se evalua el estado y se pinta vida o muerte
      ifelse erasing? [ cell-death ] [ cell-birth ]]
    display ;Muestsra los resultados
  ]
  [ set erasing? 0 ]
end
